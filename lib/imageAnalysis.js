...
/**
 * Validate response structure from Claude AI API
...
function validateResponseStructure(response) {
  ...
  // If error exists, should have details
  if (response.error && !response.details && !response.message) {
    console.error('Error response missing details');
    return false;
  }
  ...
  if (response.analysis) {
    if (typeof response.analysis !== 'object') {
      console.error('Analysis field is not an object');
      return false;
    }
  }
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      try {
        ...
        // Validate JSON structure - check if it's a proper response
        if (!validateResponseStructure(result)) {
          console.error('Invalid response structure:', result);
          throw new Error('Response structure validation failed - malformed AI response');
        }
        ...
        // This is a retryable error - malformed JSON from AI
        if (parseError.message.includes('structure validation failed')) {
          throw new Error(`Malformed AI response - will retry: ${parseError.message}`);
        }
      ...
      if (
        ...
        error.message.includes('Invalid JSON response from server')  // Don't retry syntax errors
      ) {
        throw error;
      }
      
      // DO retry for malformed AI responses (structure validation failures)
      const isRetryableAIError = error.message.includes('Malformed AI response') || 
                                error.message.includes('structure validation failed');
      
      if (!isRetryableAIError) {
        // For non-AI structure errors, check if it's a retryable server error
