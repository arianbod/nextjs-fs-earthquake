...
// ============================================================================
// Damage Assessment Schema (for post-earthquake damage analysis)
// ============================================================================

export const DamageSeverityEnum = z.enum(['low', 'moderate', 'high', 'severe']);

export const DamageAssessmentSchema = z.object({
  severity: DamageSeverityEnum.describe('Overall damage severity level'),
  summary: z.string().describe('Brief summary of the damage assessment (1-2 sentences)'),

  damageTypes: z.array(z.object({
    type: z.string().describe('Type of damage (e.g., cracks, tilting, spalling, collapse)'),
    location: z.string().describe('Location of damage (e.g., exterior wall, foundation, roof)'),
    severity: DamageSeverityEnum.describe('Severity of this specific damage'),
    description: z.string().describe('Description of the damage'),
  })).describe('List of identified damage types'),

  structuralConcerns: z.array(z.string()).describe('List of structural safety concerns'),

  immediateActions: z.array(z.string()).describe('Immediate actions to take'),

  recommendations: z.array(z.string()).describe('Recommended next steps'),

  safetyStatus: z.object({
    safeToEnter: z.enum(['yes', 'no', 'caution', 'unknown']).describe('Whether building is safe to enter'),
    evacuationRecommended: z.boolean().describe('Whether evacuation is recommended'),
    professionalInspectionNeeded: z.boolean().describe('Whether professional inspection is needed'),
  }),

  affectedSystems: z.object({
    structural: z.boolean().describe('Structural system affected'),
    electrical: z.enum(['check', 'appears_ok', 'unknown']).describe('Electrical system status'),
    plumbing: z.enum(['check', 'appears_ok', 'unknown']).describe('Plumbing system status'),
    gas: z.enum(['check', 'appears_ok', 'unknown']).describe('Gas system status'),
  }),

  confidence: ConfidenceEnum.describe('Analysis confidence level'),

  disclaimer: z.string().default('This AI assessment is preliminary only. It cannot replace professional structural engineering inspection. Do not enter damaged buildings without professional clearance.'),
});
