/**
 * Usage Tracking Service for QuakeWise Internal API
...
import { SERVICE_TIERS } from '../../config/internal-services.js';
 ...
 * Note: For pre-registered services, use the seed script instead (npm run db:seed)
 */
export async function registerApp({ appId, name, tier = 'DEV_TESTING', platformToken }) {
  try {
    // Hash the service token
    ...
    if (!tierConfig) {
      throw new Error(`Invalid tier: ${tier}. Valid tiers: WEB_APP, BATCH_JOB, DEV_TESTING`);
    }
...
/**
 * Updates service tier and rate limits
 */
