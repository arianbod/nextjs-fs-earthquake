 ...
 * @param {string} assessmentId - Assessment ID
 * @param {object} imagery - Object containing streetViewImages (with base64) and satelliteBase64
...
export async function saveGoogleImagery(assessmentId, imagery) {
	try {
		...
		const { streetViewImages, satelliteBase64, satelliteUrl, latitude, longitude } = imagery;

		// Use base64 if available, fall back to URL
		const satelliteData = satelliteBase64 || satelliteUrl;
		...
		// Add street view images (now with base64 data)
		if (streetViewImages && streetViewImages.length > 0) {
			streetViewImages
				.filter((img) => img.available && (img.base64 || img.url))
				.forEach((img) => {
					imagesToCreate.push({
						assessmentId,
						imageType: 'STREET_VIEW',
						imageData: img.base64 || img.url, // Prefer base64
						angle: img.heading || null,
						description: img.angle || `Street View ${img.heading || 0}°`,
						capturedAt: new Date(),
					});
		...
		// Add satellite image (now with base64 data)
		if (satelliteData) {
			imagesToCreate.push({
				assessmentId,
				imageType: 'SATELLITE',
