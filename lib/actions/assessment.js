      ...
      prisma.assessment.findMany({
        ...
        include: {
          ...
          images: {
            ...
            take: 3, // Get up to 3 images for variety
            select: {
              id: true,
              imageType: true,
              imageData: true,
...
export async function updateAssessment(id, data) {
  try {
    ...
    if (aiPhotoAnalysis !== undefined) updateData.aiPhotoAnalysis = aiPhotoAnalysis;
...
export async function saveBuildingInfo(assessmentId, buildingData) {
  try {
    ...
    // Update step if moving forward (4 steps total in current flow)
    if (assessment.currentStep < 4) {
      await prisma.assessment.update({
        where: { id: assessmentId },
        data: { currentStep: 4 },
      });
...
export async function saveSafetyResult(assessmentId, resultData) {
  try {
    ...
    // Calculate riskLevel and safetyRating from overallScore if not provided
    const score = parseFloat(resultData.overallScore) || 0;
    const riskLevel = resultData.riskLevel || (
      score >= 80 ? 'Low' :
      score >= 60 ? 'Moderate' :
      score >= 40 ? 'High' : 'Very High'
    );
    const safetyRating = resultData.safetyRating || (
      score >= 80 ? 'A' :
      score >= 60 ? 'B' :
      score >= 40 ? 'C' :
      score >= 20 ? 'D' : 'E'
...
/**
 * Get the user's most recent draft or in-progress assessment
 * @returns {Promise<{success: boolean, assessment?: object, error?: string}>}
 */
export async function getLastDraftAssessment() {
  try {
    const { userId } = await auth();
    ...
    console.error('Error fetching last draft assessment:', error);
    return { success: false, error: 'Failed to fetch draft assessment' };
  }
}

/**
 * Delete all assessments for a specific user by email (admin function)
 * This cascades to delete all related records (location, buildingInfo, images, safetyResult)
 */
export async function deleteAllUserAssessmentsByEmail(email) {
  try {
    // Find all assessments for this user by looking up their Clerk userId
    // Since we store Clerk userId, we need to find assessments by that
    ...
    // Group by userId to show what we have
    const byUser = {};
    ...
    Object.entries(byUser).forEach(([userId, items]) => {
      console.log(`UserID: ${userId} (${items.length} assessments)`);
      items.forEach(a => console.log(`  - ${a.id}: ${a.title || 'Untitled'}`));
    });
...
/**
 * Delete all assessments for the currently authenticated user
 */
export async function deleteAllMyAssessments() {
  try {
    const { userId } = await auth();
    ...
    if (count === 0) {
      return { success: true, message: 'No assessments to delete', deleted: 0 };
    }

    // Delete all assessments for this user
    // Related records (location, buildingInfo, images, safetyResult) will be cascade deleted
    const result = await prisma.assessment.deleteMany({
      where: { userId },
    });

    console.log(`Deleted ${result.count} assessments for user ${userId}`);

    return {
      success: true,
      message: `Successfully deleted ${result.count} assessments`,
      deleted: result.count
    };
  } catch (error) {
    console.error('Error deleting user assessments:', error);
    return { success: false, error: error.message };
  }
}
